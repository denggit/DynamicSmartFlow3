...
* **启动时链上对账**：恢复监控前用 Alchemy 查询我方链上持仓。若链上已归零或为粉尘，同步移除持仓且不启动监控；若链上有持仓但与内部状态不一致，按链上余额更新 `pos.total_tokens` 与 shares，避免后续交易错误。查询失败时冷却 `STARTUP_RECONCILE_RETRY_DELAY_SEC`（默认 60 秒）后重试，最多 `STARTUP_RECONCILE_MAX_RETRIES`（默认 3）次。
