...
* **首买者清仓禁止**：首个买入该代币的猎手在共振触发前已清仓，则该代币**永久禁止共振**，后续任意数量猎手买入均不再开仓。**修复**：若代币在 tracked（有仓位或 Agent 监控中），说明已共振过（含程序重启后恢复），不再误禁止；否则程序重启后 `_resonance_emitted` 内存清空会导致误判。
...
* `HUNTER_BUY`: 仅当猎手加仓金额在 0.5～5 SOL 区间时跟单，按档位加仓；低于 0.5 视为噪音不跟，高于 5 视为异常大单不跟。**单猎手模式不再添加新猎手**。
...
* **RPC 索引延迟保护**：共振场景下猎手刚买入，`start_tracking` 拉取的链上余额可能为 0（RPC 未及时索引）。首次全为 0 时延迟 `HUNTER_ZERO_RETRY_DELAY_SEC`（默认 8s）再拉一次。`ensure_fully_closed` 中若仓位建立不足 `FRESH_POSITION_RPC_GRACE_SEC`（默认 45s）且 chain_bal=0，延迟 `FRESH_POSITION_RETRY_DELAY_SEC`（默认 10s）二次确认后再决定是否删除，避免 RPC 未索引导致误删持仓、漏跟卖。
* **漏订阅兜底防误触发**：`SYNC_MIN_DELTA_RATIO=0.05`（5%），余额下降 ≥5% 才触发；二次确认（1s 后拉一次，误差 <2%）+ 三次确认（再 4s 拉一次）：若第三次接近 old_bal 说明 RPC 曾返回错误低值，不触发并修正基线；若第三次仍接近前两次才发 HUNTER_SELL。避免 RPC 节点不同步/缓存导致「猎手没卖却误判减仓」。
* **跟卖竞态修复**：`start_tracking` 在拉取链上余额前必须先建立 `hunter_map` 索引。否则 `_fetch_token_balance` 耗时期间，Monitor 若已把「猎手卖出」tx 推给 Agent，`hunter_map` 仍为空会漏跟卖。修复：先 `hunter_map[hunter].add(token)` 再 `await _fetch_token_balance`。
* **跟卖与 PnL 竞态**：`execute_follow_sell` 执行期间（Jupiter 卖出来回较慢），`pnl_monitor_loop` 可能并发运行；若代币价格归零，会命中止盈入口「持仓总值<MIN」并同步关闭，造成冗余。修复：跟卖时将 token 加入 `_tokens_in_follow_sell`，pnl 循环跳过该类 token。
* **止盈入口「持仓总值<MIN」应先以链上为准更新**：当 `total_value_sol < MIN` 且 `pos.total_tokens=0`（开仓 RPC 异常）时，链上可能实际有持仓。原逻辑直接 sync/卖出会误清正常仓位。修复：若 `chain_bal >= 1e-9`，先调用 `_sync_pos_total_from_chain` 以链上为准更新 `pos.total_tokens` 和 shares；更新后若仍为粉尘（如 price=0）才清仓，否则继续正常止盈/止损。
* **止盈/止损入口链上同步**：当 `chain_bal` 与 `pos.total_tokens` 差异 >1% 时，以链上为准调用 `_sync_pos_total_from_chain`，避免加仓 RPC 延迟/竞态导致内部持仓严重偏小、止损误卖部分仓位。
* **加仓 RPC 延迟保护**：加仓后查询链上余额时，若 `chain_bal < old_total`（RPC 可能未索引本笔加仓），不得覆盖为更小值；改用 `old_total + Jupiter 返回值` 更新，避免持仓被错误缩减导致止损误判。
* **开仓持仓为 0 时二次链上确认**：当开仓后 Jupiter/链上均返回 0 时，延迟 8s 再查一次链上；RPC 延迟可能导致误记录 0，二次确认可避免「开仓成功但持仓 0」触发止盈误关。
* **开仓后猎手底仓延迟拉取**：`start_tracking` 支持 `delay_before_fetch_sec`；开仓完成后延迟 6s（`START_TRACKING_BASELINE_DELAY_SEC`）再查猎手链上余额，确保底仓=猎手共振买入+过往持仓，避免 RPC 未索引导致底仓偏小、后续加仓/跟卖比例计算错误。
* **HUNTER_BUY 同 tx 去重**：同一笔 tx（hunter, token, signature）120 秒内只处理一次，避免 Monitor 重推或重复解析导致误加仓。
...
*示例*：买入 0.1 SOL 得 100 token → 成本 0.001 SOL/token。加仓 0.1 SOL 得 50 token → 新成本 (0.2/150)≈0.00133 SOL/token，第一次止盈触发价为 0.00133×2.5≈0.00333 SOL/token。**加仓仅受 max_sol 总投入上限约束**：max_sol 为开仓+所有加仓的累计投入上限（如 90 分开仓 0.06、加 5 次各 0.06 共 0.36 SOL），达到后不再加仓；减仓/部分止盈不减少已投入统计；**清仓后重新开仓则一切重置**。
...
* **粉尘清仓**: 若某份额卖出后剩余价值 < `MIN_SHARE_VALUE_SOL`（默认 0.0001，网费约 0.000011 SOL），直接清空该份额；止盈时若剩余价值不足也直接全仓。
* **链上余额为准**：所有卖出均先查链上余额，查到多少卖多少；查余额失败时兜底 99.9%（`SELL_BUFFER`）防超卖。
* **卖出重试**：失败后重试前会检查链上余额，避免前次交易已成功但验证超时导致的 6024 超卖错误。**无法查链上余额（429 等）时不再重试**，由上层 sync 处理，避免盲目广播浪费 RPC。重试前冷却 `TRADER_RETRY_COOLDOWN_SEC`（默认 5 秒），避免超频。进入任何卖出重试前必须：链上余额>0、卖出额度≤链上余额；若卖出后剩余为粉尘（< `DUST_TOKEN_AMOUNT_UI`）则直接清仓，与批量卖出一致。
* **防无限卖出**：止盈/跟卖/结构性风险清仓入口及卖出前均校验链上是否归零；若链上已归零或远小于内部状态，立即 sync 并返回，防止内部状态滞后导致无限循环尝试卖出浪费网络费用。
* **高频判定（买卖活动）**：拉 300 条解析后只统计 buy/sell 交易，排除纯 transfer；100 条买卖平均间隔 > 5 分钟才算非高频（`FREQUENCY_CHECK_SIG_LIMIT=300`，`_is_frequent_trader_by_buy_sell_activities`）。
* **Jupiter 自动滑点**：启用 `dynamicSlippage` + `dynamicComputeUnitLimit`，由 Jupiter 根据市场估算最优滑点，提高成交率；买入最大滑点 3%（`BUY_MAX_SLIPPAGE_BPS=300`），卖出按 5%/10%/20% 档位重试。
...
* **RPC 减压**：跟卖/止盈复用余额查询（4 次→2 次）；交易验证轮询间隔 `TRADER_VERIFY_POLL_INTERVAL_SEC`（默认 2s）；Agent sync 与 Alchemy 限流对齐。
* **验证超时二次确认**：当 tx_sig=None（验证超时）且首次 chain_after>0 时，延迟 `TRADER_CHAIN_AFTER_RETRY_DELAY_SEC`（默认 10s）再查一次链上余额；若归零或余额下降则视为已成交并同步状态，避免 RPC 延迟误判导致重复卖出。
* **分批止盈 chain_after=None 防护**：若为分批止盈（sell_amount < 95% total）且无法查链上余额（429 等），**不得**假定全仓已成交并 _sync_zero_and_close_position，否则会误删 position 而链上仍有余仓，形成「孤儿持仓」；下次共振买入时 reconcile 会把孤儿并入 total_tokens 但 average_price 不更新，导致 pnl 虚高/虚低误触止盈。此时改为延迟重试 chain_after；重试仍失败则假定分批成功并更新 pos、加入需手动核对，避免误删。
...
* **启动时链上对账**：恢复监控后，**同步**执行启动对账（阻塞至完成），再启动 Monitor/Agent/PnL。对账时用 Alchemy 查询每个持仓 token 的真实链上余额：若链上已归零或为粉尘，同步移除持仓并停止该币监控；若链上有持仓但与内部状态不一致，按链上余额更新 `pos.total_tokens` 与 shares，并立即写入 trader_state.json；若一致则无变化。确保本地 trader_state 与链上一致后再进入主循环，避免因本地记录滞后导致止盈/止损/卖出错误。查询失败时冷却 `STARTUP_RECONCILE_RETRY_DELAY_SEC`（默认 60 秒）后重试，最多 `STARTUP_RECONCILE_MAX_RETRIES`（默认 3）次；仍失败则保持原状态继续监控。**孤儿合并提示**：若 chain_bal 显著大于 pos.total_tokens（>1.5x），打 warning 提示可能混入前次误删导致的孤儿持仓，average_price 未重算，止盈/止损需留意。
* **交易确认**：买入、卖出均需链上确认。广播后轮询 get_signature_statuses（**必须传 searchTransactionHistory: true**，否则 RPC 只查最近缓存，交易稍旧即返回 null 误判失败），只有确认成功才更新状态。
* **二次验证**：初次验证超时/无响应时（RPC 限流易误判），等待 TX_VERIFY_RETRY_DELAY_SEC 后切换 Alchemy Key 再验一次。Alchemy 全部超时后，用 Helius Key 池内所有 Key 依次兜底查询（Helius 珍贵仅此兜底；部分 Key 可能 credit 耗尽，逐一试过）。
* **跟仓买入失败放弃**：仅当「确定失败」（Quote/Swap 未通过、从未广播交易）时加入放弃集；已广播但 RPC 验证超时则不加入，避免实际已买入却误放弃后续跟仓。本共振周期内 Monitor 不再触发共振，避免重试买在更高位置。
* **需手动核对汇总**：当交易已广播但 Alchemy/Helius 均无法验证（余额兜底也失败）时，该 token 加入「需手动核对」集合。每小时发送一次报告邮件，列出代币名称与地址，提示这些代币未在程序跟踪中，需**手动卖出**。环境变量 `MANUAL_VERIFY_REPORT_INTERVAL_SEC`（默认 3600）可配置。
...
* **Helius 批量拉取 429**：默认**不触发**紧急清仓（429 常为短暂限流）。冷却 120 秒后继续。若确需在 429 时紧急清仓，可设置 `HELIUS_429_EMERGENCY_CLOSE=1`。
* **卖出失败日志**：跟随/止损/止盈卖出失败时打 warning；price=0（归零）时也会触发止盈止损检查。
* **止盈 PnL 校正**：DexScreener 价格可能因 base/quote 解析、低流动性、插针等虚高。达到首档止盈门槛（≥100%）即用 Jupiter Quote 校验真实可卖价；以 Jupiter 结果为准决定是否止盈，避免 DexScreener 虚高误触发。
* **开仓/加仓以链上为准**：开仓或加仓成功后，立即查询链上余额，以链上 token 数量为准设置 `pos.total_tokens`；均价 = 总成本( SOL ) / 链上总量，确保单个 token 成本准确用于止盈止损。若链上为 0（RPC 延迟）则 5s 重试一次；仍失败则使用 Jupiter 返回值并打 warning，启动对账将校正。
* **持仓数量 floor**：存储持仓数量时向下取整（floor，默认 6 位小数），避免 round 进位导致记录 > 链上、卖出时超量失败。链上 4231.889572 若 round 为 4231.89 会大于实际；floor 后不超标。RPC 优先用 uiAmountString 获取完整精度。
* **链上余额精确获取**：`_fetch_own_token_balance` 优先用 uiAmountString，否则用 amount+decimals 精确计算，避免 uiAmount 的 JSON 精度丢失（如 6400.396606 被四舍五入为 6400.40 导致卖出超量失败）。跟卖时以链上为准且对卖出量 floor。
* **链上余额安全卖出量（SELL_BUFFER）**：RPC 查询链上余额可能存在四舍五入（如真实 25052.60 查得 25052.62），若直接用 chain_bal 作为卖出量会超量失败。所有以链上余额作为卖出数量（或上限）的地方统一使用 `_safe_sell_amount_from_chain(chain_bal)`，即 `floor(chain_bal × 0.999)`，确保卖出量略低于链上真实余额。应用于跟卖、止盈、止损、`ensure_fully_closed`、紧急/结构性/体检踢出清仓及 `_jupiter_sell_with_retry` 内部。
* **卖出失败减量重试**：全部滑点重试仍失败时，尝试将卖出量减 0.01（如 6400.40→6400.39）再试一次，应对精度导致超量失败。仅当 Jupiter 未广播时生效（已广播但验证超时不重试，避免重复卖出）。
* **跟卖失败 0.999 重试**：跟卖无 tx_sig 且二次确认后链上仍有余额时，可能为 RPC 四舍五入导致超量失败。用 `chain_after × 0.999` 再试一次卖出；成功则按正常跟卖流程更新持仓与交易记录（note：跟随卖出(0.999重试成功)）。
* **猎手 SWAP 视同卖出**：猎手用 token A 换 token B（SWAP）时，我方一律视为猎手卖出 token A，跟卖我方持有的 token A 换 SOL，不跟随换入 token B。
* **跟卖余额二次校验**：卖出前再拉一次链上余额并 cap，应对连续多笔跟卖时的链上延迟。
* **关闭前校验**：调用 `stop_tracking` 前先执行 `ensure_fully_closed`，若链上仍有持仓则清仓后再关监控，避免遗漏。链上余额为粉尘（< `DUST_TOKEN_AMOUNT_UI`，默认 1e-5）时不再尝试卖出（Jupiter 无路由），仅同步持仓并关闭。
* **实现**: 配置项 `FOLLOW_SELL_THRESHOLD=0.05`、`MIN_SELL_RATIO=0.3`；全平仓（跟随卖出或止盈清仓）后会自动调用 Agent 的 `stop_tracking` 停止该币监控。
* **加仓验证超时**：加仓时若已广播但 RPC 验证超时，会加入「需手动核对」集合（与开仓一致）， hourly 报告会汇总提醒；避免实际已买入但未更新持仓导致后续跟卖/止盈偏差。
...
交易相关代码审计报告见 [`DOCS/交易代码审计报告.md`](./交易代码审计报告.md)，包含：
- 问题清单与修复建议（高/中/低优先级）
- 已实施修复说明
- 已确认无问题的逻辑
...
# 全局 Alchemy 限流：每次 RPC 间隔至少 N 秒（含 alchemy_client 与 Trader 的广播/验证）
# 若免费版易 429，可加大: ALCHEMY_MIN_INTERVAL_SEC=1.5，默认 1.2s
...
* **原因**: Alchemy RPC 或 Jupiter API 触发限流。
* **解决**: (1) **全局 Alchemy 限流**：`src/alchemy/rate_limit.py` 串行化所有 Alchemy 调用（含 Trader 的广播、验证、decimals），每次间隔 `ALCHEMY_MIN_INTERVAL_SEC`；(2) `_get_decimals` 遇 429 时切换 Key 并返回默认 6；(3) `_jupiter_swap` 遇 429 时 backoff 后切换 Key 重试。**重要**：若只配置 1 个 Alchemy Key，429 时切换无效；须配置多个：`ALCHEMY_API_KEY=key1,key2,key3`。
