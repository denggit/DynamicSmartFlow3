...
* **新增猎手**: 池内猎手首次买入我们正在持有的 token 时，自动加入监控并触发 HUNTER_BUY，我方跟仓 0.1 SOL。1 分钟内多名新猎手加入时只跟仓一次。**开仓 10 分钟后加入的新猎手既不加入监控也不跟卖**。
...
**止损**：亏损 ≥ 30% 时全仓清仓（`STOP_LOSS_PCT=0.3`）。止盈/止损每 10 秒检测一次。
...
* **当前检测**（`risk_control.check_is_safe_token`）：(1) RugCheck 风险分 ≤2000；(2) 无 danger 级风险、非蜜罐/不可卖；(3) 铸币权/冻结权必须 Renounced；(4) 买入税 ≤25%；(5) **池子流动性 ≥ $5,000**（防撤池）；(6) **FDV ≤ 100 万美金**（不追高）；(7) **流动性/市值比 ≥ 3%**（防虚胖控盘）；(8) **Top 10 持仓**（防老鼠仓）：排除 LP 后，第 2~10 名合计 ≤ 剩余 40%，且单一地址 ≤ 剩余 10%。
