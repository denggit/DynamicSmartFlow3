...
* **启动时链上对账**：恢复监控后，对账在后台**异步**执行，不阻塞 Monitor/Agent/PnL 等模块启动。对账时用 Alchemy 查询我方链上持仓：若链上已归零或为粉尘，同步移除持仓并停止该币监控；若链上有持仓但与内部状态不一致，按链上余额更新 `pos.total_tokens` 与 shares；若一致则继续监控不做变化。查询失败时冷却 `STARTUP_RECONCILE_RETRY_DELAY_SEC`（默认 60 秒）后重试，最多 `STARTUP_RECONCILE_MAX_RETRIES`（默认 3）次；仍失败则保持原状态继续监控。
