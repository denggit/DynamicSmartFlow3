...
* **真实交易闭环**：集成 Jupiter Aggregator + Helius RPC，支持自动优先费（Auto Priority Fee），保障上链成功率。
...
* 同一代币被 3 个猎手买入。
* 或 2 个猎手买入且总分 > 160。
* 或 1 个 >90 分的大神 + 1 个普通猎手。


* **监听与交易解析（与 SmartFlow3 对齐）**:
* **WebSocket**: 使用 Helius `logsSubscribe`，`mentions` 为当前监控的猎手地址列表（最多 50），`commitment: "processed"` 以更早收到通知。
* **订阅确认**: 发送订阅后等待确认（约 5 秒内收到 `id=1` 的 result 或先收到 `logsNotification`），再进入主循环，避免漏单。
...
* `HUNTER_BUY`: 监控加仓行为，若加仓额度大，通知 Trader 加仓。
...
* **解决**: 代码已增加 `decimals = 9` 的默认兜底，并增加了 `if token_amount_ui > 0` 的检查。
