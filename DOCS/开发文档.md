...
* **解决**: 代码已增加 `decimals = 6` 的默认兜底（pump.fun 代币常见精度），遇 429/限流时不再重试 RPC 以免加剧限流；并增加了 `if token_amount_ui > 0` 的检查。
...
* **解决**: `trader.py` 已实现：(1) `_get_decimals` 遇 429 时直接返回默认 6，不再重试；(2) `_jupiter_swap` 遇 429 时先 backoff 等待（5~11 秒），再切换 Key 重试，最多 3 次。建议在 `.env` 中配置多个 `HELIUS_API_KEY`（逗号分隔）以提高成功率。
