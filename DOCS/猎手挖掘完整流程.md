...
| 步骤 | 操作 | 数据源 |
|------|------|--------|
| 1 | `getSignaturesForAddress(猎手主钱包, limit=100)` | Alchemy RPC |
| 2 | `_is_frequent_trader_by_blocktimes(sigs)`：失败率 ≥30% / 成功交易 < 10 笔 / 平均间隔 < 5 分钟 → 否决 | 本地 |
| 3 | `get_associated_token_address(猎手, token_mint)` 本地推导 ATA 地址 | 本地（无需 RPC） |
| 4 | `getSignaturesForAddress(ATA 地址, limit=50)` | Alchemy RPC |
| 5 | `fetch_parsed_transactions(ATA 签名)`：通常仅 2~10 笔，100% 为该代币 | Helius API |
| 6 | LP 预检、ROI 计算；若未达标 → 直接淘汰，节省 500 笔解析 | |

**成本**：过去每候选 100~500 笔 Helius；现在仅 2~10 笔，费用下降约 97%。
...
| `SM_LP_CHECK_TX_LIMIT` | 100 | （已弃用）ATA 模式改为拉 ATA 签名，通常 2~10 笔 |
...
| **LP 行为** | ADD/REMOVE LIQUIDITY | 若猎手曾对该代币有任何 LP 操作（加池/撤池），先拉 **100 笔**预检即淘汰，**WARNING 日志 + 立即拉黑**，永不跟仓（省后续 400 笔 API） |
