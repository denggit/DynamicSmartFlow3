...
|------|----------|----------|----------|------|
| **MODELA** | `HUNTER_MODE=MODELA`（默认） | 高收益 token 挖掘：DexScreener 热门币 → 回溯早期买家 | `data/modelA/` | hunters.json、scanned_tokens.json、wallet_blacklist.json、trading_history.json、closed_pnl.json、trader_state.json、summary_report*.json |
| **MODELB** | `HUNTER_MODE=MODELB` | SM 榜单挖掘：`wallets.txt` 手动导入 GMGN 钱包 → 逐个分析 | `data/modelB/` | wallets.txt、smart_money.json、trash_wallets.txt、trading_history.json、closed_pnl.json、trader_state.json、summary_report*.json |

交易记录、清仓记录、持仓状态及月度汇总均按当前模式放入对应目录，切换模式时互不干扰。

配置方式：在 `.env` 中设置 `HUNTER_MODE=MODELB` 切换到 MODELB。**注意：HUNTER_MODE 在进程启动时确定，修改后需重启进程生效。**
...
- **调用**：`SmartMoneySearcher.run_pipeline(dex_scanner)` → 返回符合条件的猎手列表（`services/modela/searcher.py`）
...
- **流程**：风控评测 → 评分 → 判定入库
- **分析**：`services/modelb/analyzer.py` 三维度统计（盈利力/持久力/真实性）
- **入库要求**：盈亏比≥1.5、胜率≥40%、总盈利>1SOL、单币平均持仓>5分钟、灰尘代币数<10、交易代币数≥7、清仓比例≥70%，再结合评分≥30
...
| 24h 涨幅 | 流动性筛选阶段不再过滤 | 由 modela/searcher 结合年龄判断：年龄在范围内且涨幅达标才挖；涨幅未达标不写 scanned，下次重试 |
...
评分逻辑按模式分离：`services/modela/scoring.py`（MODELA）、`services/modelb/scoring.py`（MODELB）。体检时按 `HUNTER_MODE` 自动选用对应 scorer。

**MODELA**（高收益 token 挖掘）：
```
胜率分 H (30%)：< 15% 零分，15%~35% 线性 0.5~1，≥35% 满分
盈利分 P (40%)：avg_roi_pct < 20% 零分，20%~60% 线性，≥60% 满分
盈亏比分 R (30%)：总盈亏比 < 1 零分，1~3 线性，≥ 3 满分
基础分 = H×30 + P×40 + R×30
...
| 条件 | 配置 |
|------|------|
| 盈亏比 ≥ 1.5 | `SM_MODELB_ENTRY_MIN_PNL_RATIO` |
| 胜率 ≥ 40% | `SM_MODELB_ENTRY_MIN_WIN_RATE` |
| 总盈利 > 1 SOL | `SM_MODELB_ENTRY_MIN_TOTAL_PROFIT_SOL` |
| 单币平均持仓 > 5 分钟 | `SM_MODELB_ENTRY_MIN_AVG_HOLD_SEC` (300) |
| 灰尘代币数 < 10 | `SM_MODELB_ENTRY_MAX_DUST_COUNT` |
| 交易代币数 ≥ 7 | `SM_MODELB_ENTRY_MIN_TRADE_COUNT` |
| 清仓比例 ≥ 70% | `SM_MODELB_ENTRY_MIN_CLOSED_RATIO` (0.7) |
| 单币平均收益率 > 10% | `SM_MODELB_ENTRY_MIN_AVG_ROI_PCT` (10) |
评分达标：总分 ≥ `SM_MODELB_ENTRY_MIN_SCORE` (30)
体检门槛：与入库一致（八项风控全部满足即可）。
```
...
| `SM_PNL_RATIO_ZERO` / `SM_PNL_RATIO_FULL` | 1.0 / 3.0 | 盈亏比分：< 1 零分，1~3 线性，≥ 3 满分 |
