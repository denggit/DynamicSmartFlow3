...
**入口优先**：`get_hunter_profit_on_token` 第一步拉主钱包 `SM_LP_CHECK_TX_LIMIT`(500) 笔，`hunter_had_any_lp_anywhere` / `hunter_had_any_lp_on_token` 有任意 LP（加池/撤池）→ 淘汰并加入黑名单，永不跟仓。通过后再进行 ATA/ROI 计算。
