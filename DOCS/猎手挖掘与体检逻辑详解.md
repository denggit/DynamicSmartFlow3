...
| 调用 | 参数 | 说明 |
|------|------|------|
| `get_signatures(client, hunter_address, limit=300)` | 主钱包 | FREQUENCY_CHECK_SIG_LIMIT=300 |
| `alchemy_client.fetch_parsed_transactions(sigs)` | Alchemy RPC getTransaction | 不依赖 Helius，RPC 格式解析 |

**判定** `_is_frequent_trader_by_buy_sell_activities_rpc(txs_rpc, hunter_address)`：

只统计 **买卖（swap）** 交易，排除纯 transfer。条件：

| 条件 | 配置 | 结果 |
|------|------|------|
| 买卖活动数 < 10 笔 | MIN_SUCCESSFUL_TX_FOR_FREQUENCY | 视为非高频（数据不足） |
| 买卖平均间隔 < 300 秒 | MIN_AVG_TX_INTERVAL_SEC | 否决（高频） |

买卖判定：从 RPC meta 解析 pre/post 余额，有非 IGNORE 代币的 token 变动且 |sol_change| ≥ 0.0001 SOL。
...
**入口优先**：`get_hunter_profit_on_token` 第一步拉主钱包 `FREQUENCY_CHECK_SIG_LIMIT`(300) 笔，`hunter_had_any_lp_anywhere` / `hunter_had_any_lp_on_token` 有任意 LP（加池/撤池）→ 淘汰并加入黑名单，永不跟仓。同批 300 条用于 LP 检测与买卖频率判定。通过后再进行 ATA/ROI 计算。
