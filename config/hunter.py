...
FREQUENCY_CHECK_SIG_LIMIT = 300  # 拉 300 条再过滤买卖，确保过滤后够 100 条统计
MIN_SUCCESSFUL_TX_FOR_FREQUENCY = 10
MAX_FAILURE_RATE_FOR_FREQUENCY = 0.30
MIN_AVG_TX_INTERVAL_SEC = 300  # 买卖交易平均间隔 > 5 分钟才算非高频
# 频繁交易判定：改用「平均持仓时间」，不再用「平均交易间隔」
MIN_AVG_HOLD_SEC_FOR_FREQUENCY = 300  # 单币平均持仓 > 5 分钟才算非频繁
...
# MODELB 入库要求（风控评测 → 评分 → 判定入库）
SM_MODELB_ENTRY_MIN_PNL_RATIO = 1.5
SM_MODELB_ENTRY_MIN_WIN_RATE = 0.4
SM_MODELB_ENTRY_MIN_TOTAL_PROFIT_SOL = 1.0
SM_MODELB_ENTRY_MIN_AVG_HOLD_SEC = 300  # 单币平均持仓时间 > 5 分钟
SM_MODELB_ENTRY_MAX_DUST_COUNT = 10  # 灰尘代币数 < 10
SM_MODELB_ENTRY_MIN_TRADE_COUNT = 7  # 至少交易代币数 7 个
SM_MODELB_ENTRY_MIN_CLOSED_RATIO = 0.7  # 清仓比例 ≥70% 才允许入库，<70% 禁止入库
SM_MODELB_ENTRY_MIN_AVG_ROI_PCT = 10.0  # 单币平均收益率 > 10% 才允许入库
SM_MODELB_ENTRY_MIN_SCORE = 30.0

# MODELB 体检：与入库条件一致，复用 SM_MODELB_ENTRY_* 系列配置
